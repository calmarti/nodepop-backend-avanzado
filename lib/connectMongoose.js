...
const main = async (err) => {
  await mongoose.connect(
    process.env
      .MONGODB_CONNECTION_STRING /* "mongodb://127.0.0.1:27017/nodepop" */,
    {
...
main().catch((err) => {
  //Gestión del error al intentar establecer la conexión
  console.log("Error al intentar establecer la conexión: ", err.message);
  process.exit(1);
});

mongoose.connection.on("error", (err) => {
  //Gestión de errores una vez establecida la conexión
